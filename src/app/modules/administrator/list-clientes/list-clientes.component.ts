...
export class ListClientesComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private pdtService: ProductService, private solService: SolicitudService,
     private router: Router, private creService: CrearService, private usService: UserService) { }
...
getUsers(){
  this.usService.getAllUsers().subscribe(p =>{
    this.clientes = p.filter(x => x.rol == 1)
  })
}
...
ExtraerCiudad(v: String){
  ...
  filterEvent(){
    ...
    if(CasaoApto == null){
      ...
      this.listaSol = this.solicitudes.filter(x => x.id_cliente == CasaoApto)
